...
namespace SimpleCommands.Runtime.Implementations
{
    ...
    public class CommandSuggestionDisplay : BaseCommandSuggestionDisplay
    {
        ...
        {
            for (var i = 0; i < _SuggestionButtonList.Count; i++)
                Destroy(_SuggestionButtonList[i].gameObject);

            _SuggestionButtonList.Clear();

            var displayAmount = Math.Min(_MaxSuggestionsToDisplay, _SuggestedCommandList.Count);

            for (var i = 0; i < displayAmount; i++)
            {
                var suggestionBtn = Instantiate(_SuggestionButtonTemplate);
                suggestionBtn.gameObject.SetActive(true);
                suggestionBtn.SetParent(_ScrollRect.content);

                var suggestionButtonComp = suggestionBtn.GetComponent<SuggestionButton>();

                var commandKeyBox = suggestionButtonComp.CommandKeyText;
                var commandDescBox = suggestionButtonComp.CommandDescText;

                var command = _SuggestedCommandList[i];

                var commandCol = $"<color=#{_ConsoleColours.CommandColorHex}>";
                var targetCol = $"<color=#{_ConsoleColours.TargetColourHex}>";
                var typeCol = $"<color=#{_ConsoleColours.TypeColorHex}>";
                var descCol = $"<color=#{_ConsoleColours.DescriptionColorHex}>";
                var endCol = "</color>";
                ...
                for (var j = 0; j < command.ParamInfo.Length; j++)
                {
            ...
            var scrollRectSize = _ScrollRect.GetComponent<RectTransform>().sizeDelta;
